...
const transform = (value, circleSize) => value ? (circleSize + 2 * paddingWidth) / 2  : -(circleSize + 2 * paddingWidth) / 2;
...
export class Switch extends Component {
  ...
  constructor(props, context) {
    ...
    this.state = {
      value: props.value,
      transformSwitch: new Animated.Value(transform(props.value, props.circleSize)),
  ...
  animateSwitch(value, cb = () => { }) {
    Animated.parallel([
      Animated.spring(this.state.transformSwitch, {
        toValue: transform(this.props.value, this.props.circleSize)
      }),
